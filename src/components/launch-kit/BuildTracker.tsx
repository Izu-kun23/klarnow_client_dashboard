...
export default function LaunchKitBuildTracker({ phases, project }: BuildTrackerProps) {
    ...
    const checkAndUpdateOnboarding = () => {
      if (typeof window !== 'undefined') {
        try {
          ...
          // Update the "Onboarding steps completed" checklist item in Phase 1
          if (isOnboardingComplete) {
            setLocalPhases((prev) => {
              ...
              try {
                ...
                localStorage.setItem('build_tracker_checklist_LAUNCH', JSON.stringify(checklistState))
              } catch (e) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      try {
        const saved = localStorage.getItem('build_tracker_checklist_LAUNCH')
        if (saved) {
  ...
  // Use local phases for display
  const displayPhases = localPhases

  // Check if there's an active phase waiting on client
  const waitingOnClientPhase = displayPhases.find(p => p.status === 'WAITING_ON_CLIENT')
  const inProgressPhase = displayPhases.find(p => p.status === 'IN_PROGRESS')
    ...
    <div className="bg-white min-h-screen">
      <div className="mx-auto max-w-7xl px-6 sm:px-8 lg:px-12 py-12">
        ...
        {waitingOnClientPhase && (
          <div className="mb-12 border-l-4 border-yellow-500 bg-yellow-50/30 pl-6 py-4">
        ...
        {expandedPhase !== null && (
          <div className="border-t-2 border-gray-200 pt-12 pb-8">
            {(() => {
              const phase = displayPhases.find(p => p.phase_number === expandedPhase)
        ...
        {expandedPhase === null && (
          <div className="border-t-2 border-gray-200 pt-12 text-center">
            <div className="max-w-md mx-auto">
              ...
              {inProgressPhase && (
                <button
                  ...
                  className="text-sm text-gray-900 hover:text-gray-600 underline underline-offset-4 transition-colors font-medium"
                >
                  View Current Phase →
                </button>
              )}
